...
for groupid = 1:size(regions, 1)
  ...
  if n>0
    omega(groupid, :) = gamma2/area * prod( ...
      phi( bsxfun(@rdivide, bsxfun(@minus, lims_upper(:)', x), ell(:)' ) ) ...
      - phi( bsxfun(@rdivide, bsxfun(@minus, lims_lower(:)', x), ell(:)') ) , ...
      2)';
  end
  ...
  if nargout >= 2 
    ...
    for d=1:loc_dim
      integrand = @(x) phi((lims_upper(d) - x)./ell(d)) ...
        - phi((lims_lower(d) - x)./ell(d)) ;
